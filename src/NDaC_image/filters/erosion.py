...
# Erosion filter with shape nxn. Convert each pixel to the minimum value 
# in the filter window. Minimum value is find by comparing the all the values except 
...
def get_eroded_image(img,n):
    ...
    # Copy the image to a new variable. This will be used to store the minimum pixel
    ...
    # issue when parsing through each pixel in the erosion process. The temporary element values in the 
    # array is set to 0. This will not change the result because erosion is looking for the minimum
    ...
    # Parse every pixel through a n by n filtering window for erosion process.
    new_img = erosion(new_img, imgTemp,n)
    
    return new_img

# A nxn filtering window for erosion process. 
#
# Variables:
# new_img = the array that will store the minimum value of the in each filtering window in 
#           the erosion process.
...
def erosion(new_img, img,n):
    ...
    for r in range(k, img.shape[0] - k): # Avoid the first k and the last k rows
        for c in range(k, img.shape[1] - k): # Avoid the first k and the last k elements
            ...
            # placing the minimum value for the pixel 
            minVal = np.min(temp) 
            new_img[r-k][c-k] = minVal
