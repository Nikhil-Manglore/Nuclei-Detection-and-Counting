...
def average_filter(img, n):
    ...
    # avg_img = a new numpy array that is going to keep the average value.
    avg_img = np.copy(img)
    avg_img = find_average(avg_img, imgTemp, n)

    return avg_img

# The function that finds the average of in each kernal.
#
# Input arguements:
# avg_img = array that will be returned
# img = original image with padded values at the boundaries
# n = kernel size
def find_average(avg_img, img, n):
    ...
    for r in range(k, img.shape[0] - k):
        for c in range(k, img.shape[1] - k):
            ...
            # find average value
            average = int(np.mean(temp))
            avg_img[r-k][c-k] = average
